...
##Discussion
Ecosystem reporting is vital to the development of Integrated Ecosystem Assessments (IEA), which lay out the framework for moving toward Ecosystem-Based Fishery Management (EBFM)[@Levin2009a]. Ecosystem reports inform Fishery Management Councils to contextual ecosystem information that supports single-species stock management, while also serving as a starting point for more applied IEA products like risk assessments. The key analytical foundations to all IEA products revolve around the concept of indicator change; whether in the short or long-term, although oftentimes managers are most interested in short-term, abrupt changes to indicator status [@Wagner2013]. Here we address the shortcomings of assigning significant trends to time series given common problems of small sample size and autocorrelation. Specifically, we performed a simulation experiment examining the strengths and weaknesses of four statistical models in detecting trends in time series under varying scenarios of series length, autocorrelation strength, and trend magnitude. 

Our results suggest that no statistical approach excelled under all simulated conditions, although there were standouts. For example, the GLS-B method was particularly strong in avoiding Type I error (i.e. falsely rejecting the null hypothesis in the absence of trend), but more likely to commit Type II error (i.e. failing to reject the null hypothesis in the presence of trend) in the presence of strong autocorrelation. Unsurprisingly, the Mann-Kendall test, which assumes independence of observations, performed poorly given simulations with any level of autocorrelation. The GLS test we implemented was unique in that it allowed for quadratic model fits and relied upon an information criterion based model selection approach. The GLS model-selection approach had the highest power across tests outside of scenarios with strong autocorrelation, but suffered from high Type I error when data had strong autocorrelation (performing worse than the MK test when N=10 with no trend). The MK-PW approach found the middle ground between GLS and GLS-B in most all scenarios under strong autocorrelation. 

We also focused on the ability of tests to detect weak, but empirically not uncommon, trend strengths in simulated times series, and found that under the limitations imposed by series lengths $\leq$ 30, no test was effective in detecting trend. This result supports the work of others [e.g. Wagner2013] suggesting that small changes in trend (<1% change in our case) are difficult to detect in time series regardless of autocorrelation strength for small samples sizes. 

To help ameliorate the common problem of low sample size in indicator analyses, @Wagner2013 suggest the use of Bayesian inference frameworks, which provide a more flexible approach to trend analysis than the binary outcomes of hypothesis testing. Specifically, their work cites Dynamic Linear Models (DLM) as a possible way forward for indicators with small samples size. A DLM approach allows for model coefficients (e.g. slope) to change with time while providing probabilities of rate changes. The implementation of DLM in the reporting of disparate ecosystem indicators may be hindered by lack of informed priors in all cases, although the use of uninformed priors could be a starting point. 
<!--
