...
get_error <- function(test, err_type, input, l, full = T){
 if (test == "gls"){
   gls_out <- tryCatch({
   ...
   if (err_type == "rmse"){
     pw_out <- sqrt(mean((y-pw_pred)^2))
   } else if (err_type == "mae") {
     pw_out <- mean(abs(y-pw_pred))
   }
 }
  return(pw_out)
}
...
# data was -0.262.
LTRENDweak   <- -0.262 + (0.0255 * c(1:x)) #Linear trend
...
error_results <- data.frame(matrix(ncol = 4, nrow = 0))
...
for (i in 1:nsims) {
  ...
  for (k in c('ARmedium','ARstrong','NOAR')){
    ...
    for (y in c('TEMP1','LTEMP1','LTEMP2','LTEMP3')){
      ...
      er <- data.frame(error = get_error('pw',"rmse",TEMP_TEST1, l = 30),
                 type = 'rmse',
                 series.length = as.numeric(x),
                 test = 'pw',
                       ar = k,
                       trend = y)
      assign('error_results', rbind(error_results, er))
      ...
      er <- data.frame(error = get_error('gls',"rmse",TEMP_lm, l = 30),
                       type = 'rmse',
                       series.length = as.numeric(x),
                       test = 'gls',
                       ar = k,
      ...
      for (j in c(11,21)) {
        ...
        er <- data.frame(error = get_error('pw',"rmse",TEMP_TEST2, l = length(get(y)[j:x]), full = F),
                 type = 'rmse',
                 series.length = as.numeric(length(get(y)[j:x])),
                 test = 'pw',
                       ar = k,
                       trend = y)
        assign('error_results', rbind(error_results, er))
        ...
        TEMP_lm <- tryCatch({
          ...
          if (k == "NOAR"){
            TEMP_lm <- fit_lm(dat = data.frame(series = get(y)[j:x] %>% as.numeric,
                            ...
                            spec = TRUE, trend = trend[j:x], ar = unlist(get(k)),
                            m = length(get(y)[j:x]), ARsd = ARsd)
          } else{
            TEMP_lm <- fit_lm(dat = data.frame(series = get(y)[j:x] %>% as.numeric,
                                             time = 1:length(get(y)[j:x])), trend = trend[j:x],
        ...
        er <- data.frame(error = get_error('gls',"rmse",TEMP_lm, l = length(get(y)[j:x]), full = F),
                 type = 'rmse',
                 series.length = as.numeric(length(get(y)[j:x])),
                 test = 'gls',
                       ar = k,
        ...
        T_lm <- cbind(T_lm, TEMP_lm[[1]]$pval)
...
```{r simulation results processing}
...
for (i in 1:length(results)){
  ...
  z <- tidyr::gather(z, Var, Value, p_30_pw:p_10_gls, factor_key = T)
    z$results <- results[i]
  p_result_mat[[i]] <- z
}
  ...
  dplyr::mutate(`Trend strength` = factor(`Trend strength`, 
                ...
                `AR strength` = factor(`AR strength`, 
                                       levels = c("no AR",
                                                  "medium AR",
                                                  "strong AR")))
  save(p_results, file = paste0(data.dir,"power_results","_",Sys.Date(),".Rdata"))
  
} else if (!run){
  
  load(file.path(data.dir,"power_results_2018-08-27.Rdata"))
...
for (i in ts_length){
  print(paste("series length",i))
  for (j in 1:nsims){
    #simulate
    temp <-arima.sim(ARstrong, n=i, rand.gen=rnorm, sd = ARsd)  
    
    #MannKendal with trend-free pre-whitening
    mkpw <- zyp.trend.vector(temp,method='yuepilon')
    mkpw_p <- unlist(mkpw[6])
    T_1 <- cbind(mkpw_p)
    ...
    trend <- rep(0,i)
    #print(length(trend))
    gls_mod <- fit_lm(dat = data.frame(series = temp %>% as.numeric,
                                       time = 1:length(temp)), trend = trend,
                      ar = unlist(ARstrong),m = i, ARsd = ARsd)
    
    T_lm <- gls_mod$best_lm$pval
    
    p_df <- data.frame(var = c("mkpw","mk","gls"),
                       p = c(T_1, T_2, T_lm),
                       n = i)
    print(paste("simulation",j))
    assign("notrend_arstrong",rbind(notrend_arstrong, p_df))
...
```{r power subset figure, fig.align="center", fig.asp=0.45}
load(file.path(data.dir, "strARnoTr_50_500_50_1000.Rdata"))
