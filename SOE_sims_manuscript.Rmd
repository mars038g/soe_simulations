  ...
  tryCatch(
    {
      ...
      mod <- arima(x = x.ts, order = c(a,0,0), include.mean = TRUE)
...
#estimate AR1
ar1.out <- do.call(rbind,
                   mapply(handle_arima, fields, a = 1))

#estimate AR2
ar2.out <- do.call(rbind,
                   mapply(handle_arima, fields, a = 2))
...
upper_95_iv_ar2 <- quantile(x = ar2.out$var, probs = 0.95, na.rm = TRUE)

trend.df <- data.frame(Var = c("none","weak","med","strong"),
                       Value = c(0,0.026, 0.051, 0.147))

if (run = T){
 save(mean_beta1_linear,lower_95_beta1,upper_95_beta1,
     mean_ar1,upper_95_ar1,iv_mean_ar1,upper_95_iv_ar1,
     mean_ar2_coef1,mean_ar2_coef2,iv_mean_ar2,upper_95_iv_ar2,trend.df,file = "sim_param.rdata")
}
...
beta <- ggplot(data = lin_coefficients, aes(x = abs(beta1))) +
  ...
  annotate("text", x = 0, y = 15, label = "A")
...
#         axis.title = element_text(size = 10),
#         strip.text = element_text(size = 10))  +
#   annotate("text", x = min(ar1.out$ar1, na.rm = TRUE)*1.05, y = 12, label = "B")
# 
# ar2 <- ggplot(data = ar2.out, aes(x = ar2)) +
#   geom_histogram(bins = 124, binwidth = 0.05,
#                  fill = "lightblue", color = "black",
#                  position = "dodge") +
#   scale_y_continuous(expand = c(0,0),
...
#   xlab(expression(rho)) +
#   ylab("") +
#   geom_vline(aes(xintercept = mean_ar2_coef2), col = "indianred", size = 1) +
#   theme_bw() +
#   theme(plot.title = element_blank(),
#         strip.background = element_blank(),
#         axis.text = element_text(size = 9),
#         axis.title = element_text(size = 10),
#         strip.text = element_text(size = 10))  +
#   annotate("text", x = min(ar2.out$ar2, na.rm = TRUE)*1.05, y = 15, label = "C")
...
iv <- ggplot(data = ar1.out, aes(x = var)) +
  ...
  theme(plot.title = element_blank(),
        ...
        axis.title = element_text(size = 10),
        strip.text = element_text(size = 10)) +
  annotate("text", x = 0, y = 15, label = "B")
...
set_levels <- function(df){
  df$trend <- df %>%
                ...
                factor(., levels=c('Strong trend','Medium trend','Weak trend','No trend'))
...
out_ar1 <- set_levels(out_ar1)
var2_ar1 <- set_levels(var2_ar1)
out_ar2 <- set_levels(out_ar2)
null_case <- set_levels(null_case)


#Aggregate p values
p_agg <- function(df){
  df %>% 
    group_by(test, series.length, rho1, trend) %>% 
    dplyr::summarise(prop = length(p[p < 0.05])/n())
}

agg_out_ar1 <- p_agg(out_ar1)
agg_var2_ar1 <- p_agg(var2_ar1)
agg_out_ar2 <- p_agg(out_ar2) #Only one level of rho in AR(2); extra grouping doesn't matter
agg_null_case <- p_agg(null_case)


#some values for text
gls <- agg_out_ar1 %>% dplyr::filter(rho1 == 0, test == "gls")
pw <- agg_out_ar1 %>% filter(rho1 == 0, test == "pw")

perc_change1 <- abs(mean(pw$prop) - mean(gls$prop))/mean(gls$prop) * 100
nom_p <- agg_out_ar1 %>% filter(trend == "No trend", rho1 == 0, series.length == 10)
strAR_noTR <- agg_out_ar1 %>% filter(trend  == "No trend", rho1 == max(rho1), series.length == 30)
...
#sample size effect
ss1 <- agg_out_ar1 %>% filter(trend  == "No trend", rho1 == max(rho1), series.length == 10)
ss2 <- agg_out_ar1 %>% filter(trend  == "No trend", rho1 == max(rho1),  series.length == 30)
...
p_agg <- out_ar1 %>% group_by(test, series.length, trend)
