...
\begin{enumerate}
\item \textit{Removal of trend} - The slope of trend $\alpha_{1}$ is estimated using the Theil-Sen estimator (Theil 1950a-c; Sen 1968) and removed from sample data if different from zero, where $\alpha_{1}$ is given by
$$\alpha_{1} = \mathrm{Median}\left (\frac{y_{j} - y_{i}}{j - l}\right)\forall l < j .$$
Trend $\alpha_{1}$ is removed from the series by
$$y_{t}^{'} = y_{t} - \alpha_{1}t,$$
...
\item \textit{Blending trend and residual series} - Trend $\alpha_{1}$ is added to the independent residual series $Y_{t}^{'}$ by 
$$Y_{t} = Y_{t}^{'} + \alpha_{1}t.$$
\item \textit{MK test} - Trend is assessed through the application of the Mann Kendall test as discussed above.

\end{enumerate}

```{r render list, results='asis', eval = F, echo = F}

if(knitr::is_latex_output()){
  cat("\\begin{enumerate}\\item \\textit{Removal of trend} - The slope of trend $\alpha_{1}$ is estimated using the Theil-Sen estimator (Theil 1950a-c; Sen 1968) and removed from sample data if different from zero, where $b$ is given by
...
```{r rmse, fig.align="center", echo = F, fig.align = "center", fig.cap="Barplots showing deviation of assigned trends from GLS and MK-TFPW (i.e. Sen's slope) procedures from true simulated trends ($\\alpha_{1} = 0, 0.026, 0.51, 0.147$) under varying autocorrelation scenarios $(\\rho = 0, 0.43, 0.8)$.\\label{Fig4}"}
