...
##Abstract
In Ecosystem-Based Fishery Management the identification of trends in ecosystem indicators is a favorable outcome for management action, although oftentimes assumptions of trend models are not properly accounted for in the reporting process. To explore the limitations of trend analysis of short times series, we applied three common methods of trend detection, including a generalized least squares model selection approach, the Mann-Kendall test, and Mann-Kendall test with trend-free pre-whitening to simulated time series of varying trend and autocorrelation strengths. Our results suggest that the ability to detect trends in time series is hampered by the influence of autocorrelated residuals when series length is small. While it is known that tests designed to account for autocorrelation will approach nominal rejection rates as series lengths increase, the series lengths incorporated in this study (N = 10, 20, 30) bias rejection rates in the presence of weak autocorrelation. This work has implications for Ecosystem-Based Fisheries Management, where indicator time series are often limited in length, maintain a variety of error structures, and a single statistical method is typically applied to all time series. If a hypothesis testing approach for indicator trend analysis is to be implemented, we suggest first characterizing candidate series based on suitability (e.g. based on variance, autocorrelation, and series length) rather than a wide-scale application of test for trend. A parametric approach to trend assessment could then be used to provide estimates of uncertainty and trend strengths from probability distributions. 
...
```{r rmse, fig.align="center", echo = F, fig.align = "center", fig.cap="Barplots showing deviation of assigned trends from GLS and MK-TFPW (i.e. Sen's slope) procedures from true simulated trends ($\\alpha_{1} = 0, 0.026, 0.51, 0.147$) by root-mean-square error (RMSE) under varying autocorrelation scenarios $(\\rho = 0, 0.43, 0.8)$.\\label{Fig4}"}
