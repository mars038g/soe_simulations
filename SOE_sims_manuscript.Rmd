...
#Estimate AR2 by conditional sum of squares

# ar.apply <- safely(arima, otherwise = NA_real_)
#   out <- ar.apply(x = x.ts, order = c(2,0,0), include.mean = TRUE, method = "CSS")
#   if (!is.null(out[[1]])) ar2 <- out$result$coef[2]
# 
# ar2.out <- SOE.data %>%
#     group_by(Var) %>%
#     arrange(Time) %>%
#     filter(Time > 1968) %>%
#     dplyr::summarise(ar2 = ar.apply(x = Value,
#                                     order = c(2,0,0),
#                                     include.mean = TRUE,
#                                     method = "CSS")$result$coef[2])
...
iv_mean <- mean(arima_lin_df$Variance)
upper_95_iv <- quantile(x = arima_lin_df$Variance, probs = 0.95)
...
iv <- ggplot(data = arima_lin_df, aes(x = Variance)) +
  ...
  ylab("") +
  geom_vline(aes(xintercept = iv_mean), col = "indianred", size = 1) +
  geom_vline(aes(xintercept = upper_95_iv), col = "indianred", size = 1, linetype = "dashed") +
  theme_bw() +
  theme(plot.title = element_blank(),
        strip.background = element_blank(),
        axis.text = element_text(size = 9),
