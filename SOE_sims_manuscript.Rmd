...
beta <- ggplot(data = lin_coefficients, aes(x = abs(beta1))) +
    geom_histogram(bins = 40, 
                 ...
                 position = "dodge",size = 0.01, alpha = 0.75) +
...
```
  ...
  The GLS procedure performed the best under the no trend and strong autocorrelation scenario: when N = 30, the rejection rate for the GLS was `r strAR_noTR[strAR_noTR$Method == "GLS",]$prop`; `r perc_change_pw`% and `r perc_change_mk`% lower than the MK-TFPW and MK tests respectively. The performance of the GLS test was also more strongly affected by sample size than the MK-TFPW test. When there was strong autocorrelation and no trend, rejection rates of the MK-TFPW test decreased only `r abs(pw_ss)`% between N = 10 and N = 30. Under the same conditions and GLS approach, rejection rates decreased by `r gls_ss`%. However, the GLS approach also performed the worst under no trend and strong autocorrelation when N = 10. This shows that while there was improvement between both tests as series lengths increased, neither test was effective in accounting for biases of autocorrelation when $N \leq 30$.
...
ggplot(agg_out_ar2,aes(color = Method, y = prop, 
                  x = series.length)) +
  geom_bar(aes(fill = Method), stat = "identity",  position="dodge",
            size = 0.5, color = "black") +
...
## Discussion
Ecosystem reporting is vital to the development of Integrated Ecosystem Assessments (IEA), which lay out the framework for moving toward Ecosystem-Based Fishery Management (EBFM)[@Levin2009a]. The key analytical foundations to all IEA products revolve around the concept of indicator change; with managers most interested in short-term, abrupt changes to indicator status [@Wagner2013]. However, the capacity for analysts to deliver rigorous and actionable insights is made more complicated by the nature of ecosystem data, which tends to a) be autocorrelated in time and space [@beale2010], and b) lie within systems changing rapidly in response to climate change and/or anthropogenic pressures [@kleisner2017]. Whether through qualitative [e.g. @zador2017] or quantitative [e.g. @plaganyi2014] methodologies for incorporating ecosystem information into management decisions, the barrier to entry for management action based on ecosystem indicators is one of change. Here we addressed the shortcomings of assigning significant trends to indicator time series given the common problems of small sample size and autocorrelation. Our results show that statistical methods commonly used to detect trend in the presence of autocorrelated residuals may be misleading when applied to short time series. 
