...
if (run = T){
 save(mean_beta1_linear,lower_95_beta1,upper_95_beta1,
     ...
     mean_ar2_coef1,mean_ar2_coef2,iv_mean_ar2,upper_95_iv_ar2,file = "sim_param.rdata")
}
...
sim_results <-data.frame(matrix(ncol = 11, nrow = 0))
names(sim_results) <- c("test","series.length", "n","trend", "ar", "mae", "rmse", "p","mean_pred","mean_true","var")
...
for (m in c(10,20,30)){
  ...
  for (k in c("notrend","ltrendweak","ltrendmed","ltrendstrong")){
    ...
    for (j in c("strongAR","medAR","NOAR")){
      ...
      for (v in c("ARsd","ARsd_up95")){
        print(paste("m =",m, "k =",k,"j =",j,"v =",v))
        for (i in 1:n){
          
          print(i)
          #generate simulations
          dat <- arima.sim(list(ar = get(j)), n=m, rand.gen=rnorm, sd = get(v))
          if (j != "NOAR"){
            dat[1] <- rnorm(1,mean=0,sd=sqrt((get(v)^2)/(1-get(j)^2)))
          }
          ...
          if (j == "NOAR"){
            gls_sim <- fit_lm(dat = dat, spec = TRUE, trend = trend,
                              ar = unlist(get(k)), m = x, ARsd = get(v))
          } else {
